...
| # | סקשן | מה משתנה |
|---|---|---|
| 5 | new perfume | עיצוב חדש לבושם how-many (תמונה חדשה אחת במקום גלריית 5 thumbs) + **סקשן בושם חדש "סבלנות"** (הקולפן) — כפתור "עוד על הבושם באתר" → עמוד המוצר (נוסף 2026-08-18; במקור בלי CTA כי לא היה עמוד) |
...
| 3 | עמוד ‎/sponsors/soos/‎ | ✅ 2026-08-04 (קומיט `b7dfe50`) — `sponsors/soos/index.html`: פאנל `#EEF0EF` בסגנון משפחת artist-talk (צ'יפ→לוקאפ→גוף 3 פסקאות→divider דסקטופ→קרדיט; hero 492×744 עם לוקאפ לבן ממורכז) + רצועת גלריה 4 תמונות על לבן (רביעית צרה). חריגת ה-lowercase המוכרעת יושמה (`.brand`); לוגו SVG inline בשם `soos-logo` (לקח התנגשות `.logo` של site-chrome — ב-lessons). דאטה: `data/sponsors.json` חדש + חוזה CLAUDE.md §6 + שורת §4 + sitemap + FIGMA_LINKS + OG. אומת מול רינדורי frames (דסקטופ 1440 + מובייל 390 ב-iframe). סקשן ההומפייג' המקשר = פרומפט 11 (⏳). |
...
| 5 | בשמים (עדכון + חדש) | ✅ 2026-08-04 — `#perfume-promo` עוצב מחדש (תמונה אחת `how-many-v2.{webp,avif}` +480/768, הוסרו 5 ה-thumbs + סקריפט ההחלפה + `pp-bio`; perfume-1..5 בדיסק) + סקשן חדש `#perfume-savlanut` ("סבלנות"/הקולפן, `savlanut.{webp,avif}` 931×934, פס EXHIBITION•VOLUME 2, "תערוכת הקולפן" מקושרת). דסקטופ = two-col, savlanut ב-`row-reverse`. כיול מובייל 36→26/20→16 + `.mbr`. דאטה: `homepage.json::perfume_promo`. אומת מול רינדורי frames + צילומי headless 1440/390. **2026-08-18:** סבלנות קיבלה כפתור "עוד על הבושם באתר" → [עמוד המוצר](https://www.zrp.co.il/product/parfume-incense-amber-and-leather) (החליף את הקו + "בלעדי לרכישה בגלריה בכיכר המדינה" — לעמוד יש עכשיו עמוד באתר הבישום). |
| 6 | upcoming section חדש | ✅ 2026-08-04 — הסקשן הוחלף כליל: כותרת מוערמת (58/91 → מובייל 46/72, חפיפה ‎-16/-12px), 2 פוסטרים סטטיים `images/homepage/upcoming/poster-{dizengoff,hamedina}` (+`poster-bg` שכבת cover מאחורי דיזנגוף per ה-fills; contain, aspect `175/301`, דסקטופ 380px/gap 16, מובייל gap 8) + כפתור `view all`; הכל → `events/`. הוסרו: עיגול+קרוסלה+חיצים+`#events-upcoming-data`+IIFE+מנגנון `hidden`, וגם **Solway מ-index.html** (שימש רק כאן; הקבצים ב-`פונטים/` נשארו). רינדור nodes ‏1318:482/3259 אומת (הטקסט אפוי בפוסטרים). אומת בדפדפן ב-1440 + מובייל (DOM metrics + צילומים; הפוסטרים 941×1672/1620×2880 + וריאנטים). CLAUDE.md §4 + FIGMA_LINKS v3. |
...
| 8 | the art works שחמט | ✅ 2026-08-04 — `.aw-grid` שחמט: מובייל 2×3 זרימה טבעית (תאים ~179px), דסקטופ 3×2 (`grid-area` ב-`min-width:769px`, I·T·I/T·I·T); 3 תמונות `images/homepage/artworks/aw-{1,2,3}` contain (+480w, avif); טקסטים 36→26px מובייל / clamp→64 דסקטופ; chevron ימינה אחרי "more"; curator: role מעל name + "read more" (4px 16px, border 1px); `works.json works[]` 4→3; tiles ישנים בדיסק. אומת בצילומי headless (⚠️ ל-Chrome headless מינימום רוחב חלון 500 — לצלם מובייל ב-500, לא 390). **המשך 2026-08-04 — תיקון משתמש: הדסקטופ יושר ל-Figma `1124:988`** (פריים דסקטופ קיים שהמשתמש הצביע עליו): שחמט 2×3 גם בדסקטופ (בוטל ה-3×2), תאי 624×600, תמונות דסקטופ נפרדות cover (`aw-*-desktop`, רינדורי `1124:991/996/997`, `<picture media>`), "‹ more" chevron לפני הטקסט, curator name→role 64px + "view" (מובייל נשאר role→name + "read more"). פרטים בשורת ההומפייג' ב-CLAUDE.md §4. **המשך-2 2026-08-04 — תמונות הדסקטופ הוחלפו לטריו החדש (בקשת משתמש):** `1124:988` = פריים ישן עם התמונות הישנות בתאים → layout בלבד ממנו; שני ה-breakpoints מגישים `aw-{1,2,3}` (הוסרו `source media`, cover ב-CSS, +`aw-1-1248w`/`aw-2-1248w`/`aw-3-690w` לרטינה, `object-position` לתאים 1/4). |
| 9 | x our artists קרוסלת פוקוס | ✅ 2026-08-05 — קרוסלת פוקוס בשני ה-breakpoints דרך קומפוננטת `.tri` הקיימת (כלל-זהב 11): המובייל כבר היה בנוי (38 שקופיות, `data-tri-start="13"` = nir giorgio levin במרכז per הפריים, captions מקושרים, בלי dots); הושלם הדסקטופ — **גריד ה-38 נמחק** מה-markup, ה-tri הופעל בשני המסכים (דסקטופ: מרכז 481×726, peeks 302×453 op‏ .6, frame 726px) + **חיצי ig-nav** (only-desktop) שמשגרים ArrowLeft/Right ל-tri ב-IIFE זעיר. caption אפור `#989898` מקושר לדף האומן (artist-linking ✓). |
| 10 | press-9 + כפתור | ✅ 2026-08-04 — `#press` הוחלף ל-9 כרטיסים סטטיים בעיצוב החדש (Figma `1322:258`; אנטומיית `/press/`, אותם כיולים), כפתור `.press-viewall` "more press & events" → `press/`. הוסרו `#press-data`+renderer+טוסט (מצב-הביניים). `press.json homepage_visible` ×9 + `homepage.json item_ids`+`view_all_route`. הסקרים על תמונת רשת13 בפריים = leftover (כמו בפריים `/press/` המובייל) — לא מומש. |
| 11 | סקשן soos בהומפייג' | ✅ 2026-08-04 — `#soos` ב-`index.html`, **מעל `#tribe-teaser`** (הכרעת משתמש — גוברת על "אחרי `#galleries`" שבפרומפט; המיקום הסופי עדיין כפוף לפרומפט ה-reorder). מובייל = Figma `1318:316` verbatim (לוקאפ 36/36/לוגו 88, קרדיטים 16 gap 8, רצועה 100.25px gap 4, כפתור 4/16); דסקטופ = אדפטציה — רצועת thumbs בגאומטריית tribe-teaser (158px, gap 16, אותו aspect .825). מיחזור מלא מנכסי עמוד הספונסר (gallery-01..03 + SVG inline). `homepage.json :: soos_sponsor` חדש. אומת בצילומי headless (מובייל in-situ מעל tribe; דסקטופ מבודד). |
| 12 | reorder + סנכרונים + חיווט | ✅ 2026-08-05 — **ה-reorder כבר היה נכון** (הסוכנים מיקמו את הסקשנים בסדר היעד תוך כדי בנייה — אומת 1:1 מול ילדי `1318:363`). סנכרונים: (א) tribe-teaser כבר תאם (כותרת מוערמת, 2 שורות קרדיטים מקושרות עם `|`, 5 thumbs, בלי כפתור); הלוגו בפריים (`b65146f9`, רונדר) = **אותו Moët & Chandon** — הלוקאפ הקיים `sponsored-by-moet.webp` שקול, לא הוחלף. (ב) ארכיון: תוקן ״בדידות **בתוך** סביבה תוססת״ בכרטיס (+10 מופעים ב-tribe titles/aria — הפריים שיחזר את הטעות המוכרעת). (ג) אינסטגרם: שתי שורות הכותרת הפכו Bold per הפריים (`1318:637` — טקסט אחד Bold); חיצים/רצועה כבר תאמו. (ד) ex-now רוחב שווה נשמר. חיווט nav→`press/` כבר בוצע בפרומפט 2. FIGMA_LINKS: `1318-363` סומן כמקור הקנוני של ההומפייג' המובייל. |
