...
**הפער:** לשניהם אין כרטיס ב-`/press/` (אי אפשר לקשר — אין route), ולכן ברגע
שהתאריך יעבור הם ייעלמו משני העמודים. שתי דרכים לסגור:
1. לבנות להם דף אירוע (צריך פריימי דסקטופ+מובייל מהמעצבת) → להסיר `soon`,
   למלא `route`, להוסיף כרטיס ב-`/press/` וב-`sitemap.xml`.
2. או להוסיף כרטיס `pcard--soon` ב-`/press/` (תקדים tal-nehoray) כדי שהם
   לפחות יישארו בארכיון.
