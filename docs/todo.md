...
- [ ] **הביו של קורין בעמוד האודות מדבר על "העבודות בתערוכה... סקלה של בדידות" (2026-09-01):** גם בפריימי הרה-דיזיין החדשים של `/about/` (`1699:1523` דסקטופ / `1699:1378` מובייל) הפסקאות 4-5 של הביו הן טקסט-אוצרת מתערוכת loneliness ("מכלול העבודות בתערוכה מהוות עבור אברהם סקלה של בדידות...") — לא מתאים להקשר עמוד אודות כללי. הועתק verbatim (וכך היה גם בגרסה הקודמת של העמוד); לוודא מול המעצבת אם מכוון.
...
- [x] ~~`images/brand/logo.svg` עוד לא קיים.~~ **✅ קיים ומקומיט (177×37 vectorized wordmark); מ-2026-09-01 משמש גם כלוגו הגדול (446px) בסקשן הסגירה של `/about/`.**
...
## 2026-09-01 — שני אירועי הרשימה החדשים בלי דף

`events/` מציג עכשיו שני כרטיסים `soon:true` שאין להם דף אירוע (אין פריימי דף בפיגמה,
רק כרטיס ברשימה):

- **`sumii-opening`** — ערב הפתיחה של ה-POP UP ART RESIDENCY של SUMII, 2.9.2026,
  כיכר דיזינגוף. Figma `1318:3205` דסקטופ / `1732:237` מובייל.
- **`nir-giorgio-levin-medina`** — שיחת אמנים עם ניר ג׳ורג׳יו לוין, 8.9.2026,
  כיכר המדינה. Figma `1471:256` / `1732:732`.

**נסגר חלקית באותו יום:** לשניהם נוסף כרטיס `pcard--soon` ב-`/press/`
(תקדים tal-nehoray) עם `data-kind="event"`+`data-date`, כך שהם יצוצו בארכיון
מעצמם ביום שאחרי האירוע ולא ייעלמו מהאתר.

**מה עוד פתוח:** אין להם דף אירוע. כשיתקבלו פריימי דסקטופ+מובייל מהמעצבת —
לבנות דף, להסיר `soon` מ-`events.json` ומהמירור `#events-list-data`, למלא
`route` ב-`events.json` וב-`press.json`, להפוך את כרטיס ה-`/press/` לקישור
רגיל, ולהוסיף ל-`sitemap.xml`.

⚠️ `nir-giorgio-levin-medina` יושב על אותו תאריך+גלריה של `events/amnon-lipkin/`
שנמחק ב-2026-08-28. זהו אירוע אחר (imageRef שונה), נוסף בהכרעת משתמש —
**אין להחזיר את amnon-lipkin.**
